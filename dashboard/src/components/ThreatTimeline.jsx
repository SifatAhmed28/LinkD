...
const LEVELS = [
  ...
  {
    key: 'L1_BLOCKLIST',
    label: 'Level 1 — Phishing Blocklist',
    icon: '🚫',
    desc: 'SHA-256 hash match against OpenPhish + PhishTank feeds',
  },
  {
    key: 'L1_CACHE',
...
// ── Ordered pipeline including blocklist ──────────────────────────────────────
const PIPELINE = ['L1_WHITELIST', 'L1_BLOCKLIST', 'L1_CACHE', 'L2_HEURISTICS', 'L3_ML'];
  ...
  return [
    ...
    },
    {
      ...
      note: f.whitelistPartialMatch
        ? 'Multi-level subdomain of whitelisted domain — sent to L2'
        : 'Single-level subdomain — trusted',
    },
    {
...
function buildL1BlocklistFeatures(result) {
  const caught = result?.level_caught === 'L1_BLOCKLIST';
  return [
    {
      label: 'URL hash in OpenPhish feed',
      ok: !caught,
      bad: caught,
      note: caught ? '🚨 URL matched phishing feed!' : 'Not found in OpenPhish',
    },
    {
      label: 'URL hash in PhishTank feed',
      ok: !caught,
      bad: caught,
      note: caught ? '🚨 URL matched phishing feed!' : 'Not found in PhishTank',
    },
    {
      label: 'SHA-256 blocklist size',
      ok: true,
      note: '300 hashes loaded from last sync',
    },
  ];
}
...
export default function ThreatTimeline({ result }) {
  ...
  const featureBuilders = {
    ...
    L1_BLOCKLIST: () => buildL1BlocklistFeatures(result),
